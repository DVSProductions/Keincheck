...
public sealed class BrokerClient : IAsyncDisposable
{
    ...
    private readonly AvaloniaMcp.Core.McpServerOptions _coreOptions;
    ...
    private BrokerClient(Avalonia.Application app, McpClientOptions options)
    {
        _app = app;
        _options = options;
        _coreOptions = options.CoreOptions ?? new AvaloniaMcp.Core.McpServerOptions();
    ...
    private async Task RunAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                Debug.WriteLine($"[AvaloniaMcp.Client] session ended: {ex.Message}");
            }
    ...
    private async Task HeartbeatAsync(PipeChannel channel, CancellationToken ct)
    {
        ...
        catch (Exception ex) { Debug.WriteLine($"[AvaloniaMcp.Client] heartbeat stopped: {ex.Message}"); }
    }
    ...
    private async Task HandleInvokeAsync(PipeChannel channel, MessageEnvelope envelope, CancellationToken ct)
    {
        ...
        {
            Debug.WriteLine($"[AvaloniaMcp.Client] failed to send result for {invoke.ToolName}: {ex.Message}");
        }
    }
