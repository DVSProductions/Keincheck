    ...
    private sealed class HostRunner : IDisposable
    {
        ...
        private void RunThread()
        {
            try
            {
                ...
                // CORE assembly (AvaloniaMcp.Core.Tools.*), so discovery targets it.
            ...
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[AvaloniaMcp] host failed: {ex}");
                Console.Error.WriteLine($"[AvaloniaMcp] host failed: {ex}");
            }
            finally
            {
                _started.Set();
