...
/// <c>AvaloniaMcp.Protocol</c> (pipe names + single-instance mutex + pipe transport) and
...
public static class Program
{
    ...
    private static async Task EnsureHubRunningAsync(string? hubPath, string pipeName, CancellationToken ct)
    {
        ...
        if (exe is null || !File.Exists(exe))
            throw new FileNotFoundException(
                ...
                "Pass --hub-exe <path> or set AVALONIAMCP_HUB_EXE.", exe ?? "(null)");
    ...
    /// <list type="number">
    ///   <item><c>AVALONIAMCP_HUB_EXE</c> environment override (explicit deployment path).</item>
    ///   <item>Co-located next to this shim (Velopack install layout).</item>
    ///   <item>Dev fallback: the sibling <c>AvaloniaMcp.Hub/bin/{Config}/net*/</c> output,
    ///         discovered by walking up from the shim's bin dir to the solution root.</item>
    /// </list>
    /// </summary>
    private static string? ResolveHubExe()
    {
        var fromEnv = Environment.GetEnvironmentVariable("AVALONIAMCP_HUB_EXE");
        ...
        //    is .../AvaloniaMcp.Connect/bin/{Config}/net8.0/ ; the Hub's is
        //    .../AvaloniaMcp.Hub/bin/{Config}/net10.0/ . Walk up to the solution dir
        //    (the one containing both project folders) and probe the Hub output.
        return ProbeDevHubExe(baseDir);
    }

    /// <summary>
    /// Walks up from <paramref name="startDir"/> looking for a sibling <c>AvaloniaMcp.Hub</c>
    ...
    {
        // From .../AvaloniaMcp.Connect/bin/{Config}/net8.0 climb until we find a directory
        // that has a sibling "AvaloniaMcp.Hub" folder (the solution root).
        var dir = new DirectoryInfo(startDir);
        for (var depth = 0; dir is not null && depth < 8; depth++, dir = dir.Parent)
        {
            var hubProject = Path.Combine(dir.FullName, "AvaloniaMcp.Hub");
    ...
    {
        foreach (var name in new[] { "AvaloniaMcp.Hub.exe", "AvaloniaMcp.Hub" })
        {
    ...
    private static void PrintUsage()
    {
        ...
        Console.Error.WriteLine(
            """
            avalonia-mcp-connect — stdio<->hub MCP bridge

            Usage:
              avalonia-mcp-connect [--pipe <name>] [--hub-exe <path>]
            ...
            Options:
              ...
              --hub-exe <path>  Hub executable to launch if no hub is running.
                                Falls back to AVALONIAMCP_HUB_EXE, a co-located hub,
    ...
    private static void Log(string message)
        => Console.Error.WriteLine($"[avalonia-mcp-connect] {message}");
}
