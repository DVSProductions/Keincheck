...
public sealed class WebSocketTokenCliTests : IDisposable
{
    ...
    [Fact]
    public void Refuses_To_Enable_The_Endpoint_On_The_Operators_Behalf()
    {
        // Exit 4 mirrors --issue-credential's "remote access is not enabled". Turning on a
        // reachable endpoint is a deliberate one-time decision, not a side effect of compiling.
        var access = HubWebSocketAccess.Open(PolicyPath);
